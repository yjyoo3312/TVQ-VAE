...
class TVQVAETrainer(object):
    ...
    def __init__(self, args):
        ...
        self.res_dir = f"results_{args.dataset}_{args.seed}_{args.init_seed}_{args.n_clusters}_{args.n_embeddings}_{args.n}"
    ...
    def initialize_vq(self, pretrained_vq_path, epochs=200):        
        ...
        with torch.no_grad():
            for batch in tqdm(dataset_loader, total=len(dataset_loader), desc="stacking encodings"):
                ...
                _, _, _, w_enc, attn_mask, _, perplexity = self.model(input_ids, attention_mask, valid_pos)  
        ...
        optimizer_tp = Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.args.lr) #bert not trained.
        self.model.train()
        self.model.ae.eval()
        ...
        for epoch in range(epochs):
            ...
            for batch in dataset_loader:
                ...
                word_dists, word_dists_o, _, _ = self.model.forward_topic(doc, o_doc, expansion=True)
            ...
            if epoch % int(epoch//4) == 0 and epoch > 0:
                print(f"Epoch {epoch} loss recon_o = {total_loss_rec_o.avg:.4f} loss recon_e = {total_loss_rec_e.avg:.4f} loss kld = {total_loss_kld.avg:.4f}")

        npmi, diversity, _,_, perplexity = self.inference(epoch, suffix="_initial")
        tq = npmi*diversity # topic quality                
        print(f"Initial npmi={npmi:.4f}: diversity={diversity:.4f}: tq={tq:.4f}: perplexity = {perplexity:.4f}") 
    ...
    def pretrain(self, train_epoch=20):
        ...
        optimizer = Adam(filter(lambda p: p.requires_grad, model.ae.parameters()), lr=self.args.lr)        
            ...
            for batch_idx, batch in enumerate(dataset_loader):                
                ...
                if batch_idx % int(len(dataset_loader)//4) == 0 and batch_idx > 0:                      
                    print(f"epoch {epoch}: idx {batch_idx}: loss_mse = {total_loss_mse.avg:.4f}: "
                          +f"loss_vq {total_loss_vq.avg:.7f}: perplexity {total_perplexity.avg:.4f}")
    ...
    def train(self, train_epoch=20):                   
        ...
        self.initialize_vq(pretrained_vq_path, epoch=-1, cluster=self.args.n_clusters)
        self.model.to(self.device)
        ...
        optimizer = Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.args.lr) 
        ...
        for epoch in range(train_epoch): 
            ...
            total_loss_rec_e, total_loss_rec_o, total_loss_kld  = AverageMeter(), AverageMeter(), AverageMeter()
            ...
            for batch_idx, batch in enumerate(dataset_loader):
                ...
                _, _, _, w_enc, attn_mask, loss_vq, perplexity = self.model(input_ids, attention_mask, valid_pos)  
                doc_enc, o_doc_enc,_ = self.utils.calc_doc_enc(w_enc, input_ids = input_ids, doc_ids=doc_ids, attn_mask=attn_mask, max_len=max_len)  
                o_doc_enc = o_doc_enc.to(self.device)

                prior_mean, prior_var, posterior_mean, posterior_var, posterior_log_var, \
                word_dists, word_dists_o, _, _ = self.model.forward_topic(doc_enc, o_doc_enc)
                ...
                if batch_idx % int(len(dataset_loader)//4) == 0 and batch_idx > 0:                  
                    print(f"Epoch {epoch}: idx {batch_idx} loss recon_e {total_loss_rec_e.avg:.4f} loss recon_o {total_loss_rec_o.avg:.4f} "
                          +f"loss kld {total_loss_kld.avg:.4f} lr {optimizer.param_groups[0]['lr']:.9f}")
            ...
            tq = npmi*diversity # topic quality                
            print(f"epoch {epoch}: npmi={npmi:.4f}: diversity={diversity:.4f}: tq={tq:.4f}: perplexity = {perplexity:.4f}")               
        ...
        print(f"Saved: npmi = {npmi_best:.4f} at epoch {best_epoch}")
    ...
    def inference(self, epoch, suffix="", test_mode=False):
        ...
        #nmi, purity = self.utils.cluster_eval(label_path=self.label_dir, preds=torch.from_numpy(self.td), split=[0,self.split[0]])        
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='nyt', help='nyt, 20ng, m10, dblp, bbc_news, imdb')
