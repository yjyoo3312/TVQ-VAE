...
class Block(nn.Module):
    ...
    def forward(self, x, token_num, logit_num, embeddings=None, layer_past=None, return_present=False):
        # TODO: check that training still works
        if return_present: assert not self.training
        # layer past: tuple of length two with B, nh, T, hs
        if embeddings is not None:
            x[:,token_num-1:,:] = x[:,token_num-1:,:] + embeddings[:,:logit_num-token_num+1,:]
...
class GPT(nn.Module):
    ...
    def forward(self, idx, embeddings=None, targets=None):
        ...
        if embeddings is not None:
            logit_num = x.shape[1]
            token_num = embeddings.shape[1]
            for block in self.blocks:
                x = block(x, token_num, logit_num, embeddings=embeddings)
        else:
            x = self.blocks(x)
