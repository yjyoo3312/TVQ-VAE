...
def custom_collate(batch):
    ...
    classes = []
    for sample in batch:
        image.append(torch.from_numpy(sample['image']))
        coord.append(torch.from_numpy(sample['coord']))
        classes.append(torch.tensor(sample['class']))
    data_dict = {'image': torch.stack(image), 'coord': torch.stack(coord), 'class': torch.stack(classes)}
    return data_dict

def custom_collate_original(batch):
